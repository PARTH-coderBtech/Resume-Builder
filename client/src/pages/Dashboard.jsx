...
const Dashboard = () => {
  ...
  const uploadResume = async (event) => {
    ...
    try {
      ...
      const {data} = await api.post('/api/ai/upload-resume',{title,resumeText: trimmedText },{headers:{
        Authorization: token
       }})
       setTitle('')
      setResume(null)
      ...
      <div className='max-w-7xl mx-auto px-4 py-8'>
        ...
        bg-clip-text text-transparent sm:hidden'>Welcome, Joe Doe</p>
         ...
         <button onClick={() => setShowCreateResume(true)}  className='w-full bg-white sm:max-w-36 h-48 flex flex-col items-center
          justify-center gap-2 rounded-lg text-slate-600 border border-dashed
          ...
          <div className='grid grid-cols-2 sm:flex flex-wrap gap-4'>
            {allResumes.map((resume,index)=>{
              ...
              return(
              <button key={index} onClick={()=> navigate(`/app/builder/${resume._id}`)} className=' group relative w-full sm:max-w-36 h-48 flex flex-col items-center justify-center
              rounded-lg gap-2 border group-hover:shadow-lg transition-all duration-300 cursor-pointer' style={{
                background: `linear-gradient(135deg, ${baseColor}10, ${baseColor}40)`, borderColor:
                baseColor + '40'
              }} >
